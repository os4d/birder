# Checks

This section provides documentation for the various checks available in Birder.

## Standard Checks

*   [AMQP](./checks/amqp.md)
*   [ElasticSearch](./checks/elastic_search.md)
*   [FTP](./checks/ftp.md)
*   [HTTP/HTTPS](./checks/http.md)
*   [LDAP/ActiveDirectory](./checks/ldap.md)
*   [Memcached](./checks/memcache.md)
*   [MongoDB](./checks/mongodb.md)
*   [MySQL/MariaDB](./checks/mysql.md)
*   [Passive](./checks/passive.md)
*   [PostgreSQL/PostGIS](./checks/pg.md)
*   [Redis](./checks/redis.md)
*   [S3](./checks/s3.md)
*   [SMTP](./checks/smtp.md)
*   [SSH](./checks/ssh.md)
*   [TCP/IP](./checks/tcp.md)

## Specialized Checks

*   [Celery](./checks/celery.md)
*   [JSON](./checks/json.md)
*   [XML](./checks/xml.md)
